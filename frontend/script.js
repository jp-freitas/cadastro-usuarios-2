...
form.addEventListener("submit", async function (event) {
  ...
  */
  const name = document.getElementById("name").valor.trim();
  console.log(name);
  console.log(name);
  /*
    LACUNA 2:
    ...
    - Use .trim()
  */
  const email = documentgetElmentById("email").valor.trim();
  console.log(email);
  console.log(email);
  /*
    LACUNA 3:
  ...
  */
  const password = document.getElementById("passaword").value;
  ...
  */
  ...
  try {
    /*
      ...
      - Use JSON.stringify()
    */
    const response = await fetch("htttp://localhost:3000/users", {
      method: POST ,
      headers: {
